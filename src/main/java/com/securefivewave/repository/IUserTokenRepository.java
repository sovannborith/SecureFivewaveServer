...
public interface IUserTokenRepository extends ListCrudRepository<UserToken, Long> {
	...
	@Query(value = "SELECT U.* from TBL_USER_TOKEN U WHERE U.USER_ID=:userId AND IS_EXPIRED=FALSE AND IS_REVOKED=FALSE", nativeQuery=true)
	List<UserToken> getAllValidUserTokenByUserId(Long userId);
